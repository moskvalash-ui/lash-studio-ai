...
test('validation states are explicit for reviewed records and the draft Eyeliner direction slot', () => {
  ...
  for (const definition of allDefinitions()) {
    ...
    if(['geometry.squirrel','geometry.doll','geometry.fox','geometry.cat','direction.cat','direction.fox','construction.root-definition','construction.angel','construction.wet','construction.wispy','preset.eyeliner'].includes(definition.id)){
...
test('legacyReference is isolated and cannot masquerade as validated professional data', () => {
  ...
  for (const definition of allDefinitions()) {
    if(!['geometry.squirrel','geometry.doll','geometry.fox','geometry.cat','construction.angel','construction.wet','construction.wispy','preset.eyeliner'].includes(definition.id))for (const field of forbiddenNumericFields) assert.strictEqual(definition.legacyReference[field], null, `${definition.id}/${field}`);
