...
test('L: unrelated production systems have zero diff against committed HEAD', () => {
  ...
  // consent-manager.test.js) — not a regression this Live Scan lifecycle
  // fix needs to guard against.
  for (const file of ['backend/worker.js', 'consent-manager.js', 'client-data-consent.js', 'lash-design-domain.js']) {
    let diff;
